 ...
 */
package com.example.grafos.pesados;

import com.example.grafos.nopesados.*;
import com.example.grafos.pesados.*;
import com.example.grafos.excepciones.*;
...
public class DigrafoPesado extends GrafoPesado{
    ...
    */
    ...
    public DigrafoPesado Dikstra3(int verticeInicial, int verticeFinal,List<Double> listaRetorno) throws ExcepcionNroVerticesInvalido, ExcepcionAristaYaExiste {
        int numVertice=listaDeAdyacencias.size();
        List<Integer> predecesor=new ArrayList<>();
        boolean[] marcados=new boolean[numVertice];
        List<Double> listaDeCostos=new ArrayList<>();
        for(int i=0;i<numVertice;i++){
            listaDeCostos.add(10000.0);
            marcados[i]=false;
            predecesor.add(-1);
        }
        listaDeCostos.set(verticeInicial,0.0);
        int VmenorCosto=verticeInicial;
        while(listaDeCostos.get(VmenorCosto)!=10000.0 &&
                marcados[verticeFinal]==false) {
            marcados[VmenorCosto]=true;
            Iterable<Integer> it=adyacentesDeVertice(VmenorCosto);
            for (Integer verticeEnTurno: it) {
        ...
        if (marcados[verticeFinal]==false) {
            return g1=new DigrafoPesado(1);
        }else{
            g1=new DigrafoPesado(cantidadDeVertices());
            listaRetorno.add(listaDeCostos.get(verticeFinal));
            int enTurno=verticeFinal;
            Stack<Integer> pila=new Stack<>();
            ...
            while (pila.size()>1) {
                aux=pila.pop();
                g1.insertarArista(aux,pila.peek(),peso(aux,pila.peek()));
            }
